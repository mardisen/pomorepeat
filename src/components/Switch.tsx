...
export const Switch: Component<Props> = (props: Props) => {
  ...
  return <div class='flex flex-row items-center gap-3'>
    <label class='relative inline-block h-5 w-9 '>
      <div
        class='bg-dblack dark:bg-dwhite rounded-full h-5 w-9 flex flex-row items-center cursor-pointer absolute transition-colors'
        classList={{ ['bg-primary-700']: props.checked }}
      >
        <div
          class='bg-dwhite dark:bg-dblack rounded-full h-4 w-4 m-1 transition-all'
    ...
    <div class='text-primary-600 text-center'>{props.text}</div>
  </div>;
};
